...
class CSV {
   ...
   */
  static parse(path, options, rowHandler) {
    // ckeck arguments
      ...
      if (err) {
        ...
        var handleRow = function() {
          ...
          if (typeof rows[++current] != 'undefined') {
            ...
            if (options.headers && current === 0) {
              return handleRow();
            }
            rowHandler(
              null,
              CSVRowToArray(rows[current], delimiter),
              handleRow,
              size
            );
          }
          ...
          // read a bit more
            ...
            fs.read(fd, buffer, 0, length, null, function(
              err,
              bytesRead,
              buffer
            ) {
              if (err) {
                ...
                // convert string to utf8
                  ...
                  strBuffer += iconv.decode(
                    bytesRead < length ? buffer.slice(0, bytesRead) : buffer,
                    charset
                  );
                }
